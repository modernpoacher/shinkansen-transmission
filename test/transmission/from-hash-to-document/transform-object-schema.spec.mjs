...
import transformObjectSchema from 'shinkansen-transmission/transmission/from-hash-to-document/transform-object-schema'

/**
 *  `transformObjectSchema` is a different entry point to `fromHashToDocument`
 *  where the `uri` and `index` values are destructured from an object in the
 *  third position
 *
 *  Otherwise the behaviour is identical, as are these tests
 *
 *  `transformObjectSchema` should be combined with `transformArraySchema` and deleted
 */

describe('shinkansen-transmission/transmission/from-hash-to-document/transform-object-schema', () => {
  before(() => {
    const {
      env: {
        DEBUG
    ...
    describe('With values', () => {
      ...
      describe('Transforming `array` type schemas', () => {
        ...
        it('transforms `array` type schemas (`items` is an array of `array` type and `items` is an object of `array` type and `items` is an object of `string` type)', () => {
          ...
          expect(transformObjectSchema(values, schema))
            .to.eql(document)
        })
  ...
  describe('Hash is undefined and schema is defined', () => {
    it('transforms `string` type schemas', () => expect(transformObjectSchema(undefined, { type: 'string' })).to.eql(undefined))
    it('transforms `number` type schemas', () => expect(transformObjectSchema(undefined, { type: 'number' })).to.eql(undefined))
    it('transforms `array` type schemas', () => expect(transformObjectSchema(undefined, { type: 'array' })).to.eql([]))
    it('transforms `object` type schemas', () => expect(transformObjectSchema(undefined, { type: 'object' })).to.eql({}))
    it('transforms `boolean` type schemas', () => expect(transformObjectSchema(undefined, { type: 'boolean' })).to.eql(undefined))
    it('transforms `null` type schemas', () => expect(transformObjectSchema(undefined, { type: 'null' })).to.eql(undefined))
  })
